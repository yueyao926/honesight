...
import "./HandDrawnPressButton.css";

export function HandDrawnPressFilters() {
  return (
    <svg className="hand-drawn-press-filter-defs" aria-hidden="true" width="0" height="0">
...
type HandDrawnPressButtonProps = ButtonHTMLAttributes<HTMLButtonElement> & {
  children: ReactNode;
};

export default function HandDrawnPressButton({
  children,
  className = "",
  type = "button",
  ...props
}: HandDrawnPressButtonProps) {
  const classes = ["hand-drawn-press-button", className].filter(Boolean).join(" ");

  return (
    <span className="hand-drawn-press-button-scaler">
      <button className={classes} type={type} {...props}>
        <svg
          className="hand-drawn-press-button__cosm"
        ...
        <svg className="hand-drawn-press-button__highlight" viewBox="0 0 144.75738 77.18431" preserveAspectRatio="none" aria-hidden="true">
        ...
        <span className="hand-drawn-press-button__label">{children}</span>
      </button>
    </span>
  );
}
