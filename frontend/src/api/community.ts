...
export type CommunityComment={id:number;content:string;parent_id?:number|null;reply_to_user_id?:number|null;like_count:number;reply_count:number;created_at:string;author:{id:number;username:string;avatar_url?:string|null}};
...
export const uploadCommunityImage=(file:File)=>{const body=new FormData();body.append("file",file);return apiRequest<{image_url:string}>("/upload/image",{method:"POST",body})};
